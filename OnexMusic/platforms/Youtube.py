...
from OnexMusic import LOGGER
from OnexMusic.utils.database import is_on_off
from OnexMusic.utils.formatters import time_to_seconds
from config import CybruxoApi_Key, CybruxoApi_Url as YTPROXY
...
class YouTubeAPI:
    ...
    async def download(
        ...
        async def audio_dl(vid_id):
            try:
                if not CybruxoApi_Key:
                    logger.error("API KEY not set in config, Set API Key you got from @Cybruxo")
                    return None
                if not YTPROXY:
                    logger.error("API Endpoint not set in config\nPlease set a valid endpoint for CybruxoApi_Url in config.")
                    return None
                
                headers = {
                    "x-api-key": f"{CybruxoApi_Key}",
