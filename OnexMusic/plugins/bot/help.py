...
from OnexMusic.utils import help_pannel
from OnexMusic.utils.database import get_lang, get_model_settings, update_model_settings
from OnexMusic.utils.decorators.language import LanguageStart, languageCB
from OnexMusic.utils.inline.help import help_back_markup, private_help_panel
from config import BANNED_USERS, START_IMG_URL, SUPPORT_CHAT, CybruxoApi_Url
import config
...
async def fetch_tts_models():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{CybruxoApi_Url}/tts/models") as response:
...
async def fetch_image_models():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{CybruxoApi_Url}/image/models") as response:
                if response.status == 200:
                    data = await response.json()
...
async def fetch_ai_models():
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{CybruxoApi_Url}/ai/models") as response:
                if response.status == 200:
                    data = await response.json()
                    return data.get("models", [])
                return []
...
async def helper_cb(client, CallbackQuery:CallbackQuery, _):
    ...
    if cb == "hb1":
        ...
        await CallbackQuery.edit_message_text(f"AI, TTS and Image Model Settings \n\n[Check Docs here]({CybruxoApi_Url}/docs)", reply_markup=InlineKeyboardMarkup(btn),parse_mode=ParseMode.DEFAULT)
    elif cb == "hb17":
